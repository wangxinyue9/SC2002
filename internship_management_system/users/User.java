...
public abstract class User {
    ...
    public List<InternshipOpportunity> applyFilterSettings(FilterSettings filtersettings){
        ...
        // display table
        printOpportunitiesTable(filtered);

        //prompt user for change in filter
    ...
    // Interactive editor for this user's filter settings using console input
    public void setFilterSettings() {
        ...
        for (int i = 0; i < stVals.length; i++) {
            System.out.println((i + 2) + ") " + stVals[i]);
        }
        System.out.print("Enter choice: ");
        int statusChoice = promptInt(scanner, 0, stVals.length + 1);
        switch (statusChoice) {
            case 0: {
                for (InternshipOpportunityStatus s : current.statuses()) { b.addStatus(s); }
                break;
            }
            case 1: {
                // no statuses added => no restriction
                break;
            }
            default: {
                int sel = statusChoice - 2;
                if (sel >= 0 && sel < stVals.length) { b.addStatus(stVals[sel]); }
                break;
            }
        }
        ...
        for (int i = 0; i < appVals.length; i++) {
            System.out.println((i + 2) + ") " + appVals[i]);
        }
        System.out.print("Enter choice: ");
        int appChoice = promptInt(scanner, 0, appVals.length + 1);
        switch (appChoice) {
            case 0: {
                for (InternshipApplicationStatus s : current.applicationStatuses()) { b.addApplicationStatus(s); }
                break;
            }
            case 1: {
                // no application statuses added => no restriction
                break;
            }
            default: {
                int sel = appChoice - 2;
                if (sel >= 0 && sel < appVals.length) { b.addApplicationStatus(appVals[sel]); }
                break;
            }
        }
        ...
        for (int i = 0; i < lvVals.length; i++) {
            System.out.println((i + 2) + ") " + lvVals[i]);
        }
        System.out.print("Enter choice: ");
        int levelChoice = promptInt(scanner, 0, lvVals.length + 1);
        switch (levelChoice) {
            case 0: {
                for (InternshipLevel l : current.levels()) { b.addLevel(l); }
                break;
            }
            case 1: {
                break;
            ...
            default: {
                int sel = levelChoice - 2;
                if (sel >= 0 && sel < lvVals.length) { b.addLevel(lvVals[sel]); }
                break;
            }
        }
        ...
        switch (majorChoice) {
            case 0: {
                for (String m : current.majors()) { b.addMajor(m); }
                break;
            }
            case 1: {
                break;
            ...
            case 2: {
                System.out.print("Majors (comma-separated): ");
                String line = scanner.nextLine();
                if (line != null && !line.isBlank()) {
                    String[] parts = line.split(",");
                    for (String p : parts) {
                        String mj = p.trim();
                        if (!mj.isEmpty()) { b.addMajor(mj); }
                    }
                }
                break;
            }
        ...
        System.out.println("0) No change");
        System.out.println("1) Only visible");
        ...
        switch (visChoice) {
            ...
            case 2: {
                b.visibleOnly(null); // no restriction
                break;
            }
            default: {
                break;
        ...
        switch (beforeChoice) {
            ...
            case 2: {
                ...
                try { b.closingBefore(LocalDate.parse(s)); } catch (Exception ignored) {}
                break;
            }
            default: { break; }
        }
        ...
        switch (afterChoice) {
            ...
            case 2: {
                System.out.print("Enter date (yyyy-MM-dd): ");
                String s = scanner.nextLine().trim();
                try { b.closingAfter(LocalDate.parse(s)); } catch (Exception ignored) {}
                break;
            }
            default: { break; }
        }
        ...
        for (int i = 0; i < sorts.length; i++) {
            System.out.println((i + 1) + ") " + sorts[i]);
        }
        System.out.print("Enter choice: ");
        ...
        if (sortChoice == 0) {
            ...
            int sel = sortChoice - 1;
            if (sel >= 0 && sel < sorts.length) { b.sortBy(sorts[sel]); }
        }
